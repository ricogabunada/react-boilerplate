...
export function getCoinPhaseSettings(options = {}, cancellable = false) {
	...
	if (cancellable) {
		return apiCore.cancellableGet(`/api/v1/coin-comp-phase-settings?${query}`);
	}

	return apiCore.get(`/api/v1/coin-comp-phase-settings?${query}`);
}

export function getCoinPhaseSetting(id) {
	return apiCore.get(`/api/v1/coin-comp-phase-settings/${id}`);
}

export function addCoinPhaseSetting(body) {
	return apiCore.post('/api/v1/coin-comp-phase-settings', body);
}

export function updateCoinPhaseSetting(body, id) {
	return apiCore.put(`/api/v1/coin-comp-phase-settings/${id}`, body);
}

export function deleteCoinPhaseSetting(id) {
	return apiCore.delete(`/api/v1/coin-comp-phase-settings/${id}`);
}

export function forceDeleteCoinPhaseSetting(id) {
	return apiCore.delete(`/api/v1/coin-comp-phase-settings/${id}?force=true`);
}

export function activateCoinPhaseSetting(id) {
	return apiCore.put(`/api/v1/coin-comp-phase-settings/${id}?activate=true`);
}

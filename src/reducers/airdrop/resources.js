...
export function getAllCoinPhases(options = {}, cancellable = false) {
	...
	if (cancellable) {
		return apiCore.cancellableGet(`/api/v1/airdrop-phases/coin?${query}`);
	}

	return apiCore.get(`/api/v1/airdrop-phases/coin?${query}`);
}
...
export function getCoinPhases(options = {}, cancellable = false) {
	...
	if (cancellable) {
		return apiCore.cancellableGet(`/api/v1/airdrop-phases?${query}`);
	}

	return apiCore.get(`/api/v1/airdrop-phases?${query}`);
}

export function getCoinPhase(id) {
	return apiCore.get(`/api/v1/airdrop-phases/${id}`);
}

export function addCoinPhase(body) {
	return apiCore.post('/api/v1/airdrop-phases', body);
}

export function updateCoinPhase(body, id) {
	return apiCore.put(`/api/v1/airdrop-phases/${id}`, body);
}

export function updateCoinPhaseStartOrEnd(id, mode) {
	return apiCore.put(`/api/v1/airdrop-phases/${id}/${mode}`);
}
